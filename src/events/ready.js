...
module.exports = async (client) => {
  ...
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`Alcatraz a quitté le serveur: ${guild.guild_name}`);
  }

  client.logger.info('Alcatraz est maintenant en ligne');
  client.logger.info(`Alcatraz est en service sur ${client.guilds.cache.size} serveur(s)`);
};
